...
const StarIcon = React.memo(
	...
	}) => <Star key={index} size={iconSize} fill={style.fill} color={style.color} onClick={onClick} onMouseMove={onMouseMove} className={cn('transition-colors duration-200', isInteractive && 'cursor-pointer')} style={style} />
);
StarIcon.displayName = 'StarIcon';

const StarRating_Fractions = ({ className, color = '#e4c616', iconSize = 24, maxStars = 5, onChange, readOnly = false, value }: StarRatingBasicProps) => {
	const [hoverRating, setHoverRating] = React.useState<number | null>(null);

	// useId ensures same prefix for server & client
	const uniqueId = React.useId();
	const starIds = React.useMemo(() => Array.from({ length: maxStars }, (_, i) => `star-${uniqueId}-${i}`), [uniqueId, maxStars]);
	...
	return (
		<div className={cn('relative flex items-center gap-x-0.5', className)} onMouseLeave={handleMouseLeave}>
			...
			{starIds.map((id, index) => {
				...
				return <StarIcon key={id} index={index} style={style} iconSize={iconSize} onClick={(e) => handleStarClick(index, e)} onMouseMove={(e) => handleStarHover(index, e)} isInteractive={!readOnly} />;
			})}
		</div>
	);
};
