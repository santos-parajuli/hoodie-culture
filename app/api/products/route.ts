...
export async function GET(req: Request) {
	...
	try {
		...
		const excludeProductCondition = { title: { $ne: 'Customized Hoodie' } };
		if (search) {
			const words = search.split(/\s+/).filter(Boolean);
			query.$and = [...(query.$and || []), ...words.map((word) => ({ title: new RegExp(word, 'i') })), excludeProductCondition];
		} else {
			query.$and = [...(query.$and || []), excludeProductCondition];
		}
